...
public class PuzzleController : MonoBehaviour
{
    ...
    public void Prepare()
    {
        ...
        GameManager.Instance.numMaxPiece = pieces.Length;
        ...
        for (int i = 0; i < pieces.Length; i++)
        {
            ...
            pieces[i].oldSize.x = pieces[i].renderer.bounds.size.x;
            pieces[i].oldSize.y = pieces[i].renderer.bounds.size.y;

            pieces[i].oldScale = pieces[i].transform.localScale;
   

            //pieces[i].spriteRenderer.drawMode = SpriteDrawMode.Tiled;
            //pieces[i].spriteRenderer.size = new Vector3(1, 1, 1);
            //pieces[i].spriteRenderer.drawMode = SpriteDrawMode.Simple;
            //Debug.Log("pieces[i].oldSize" + pieces[i].oldSize);
        }
        ...
        if (changeOnlyRotation)
        {
            ...
            for (int i = 0; i < pieces.Length; i++)
                if (_filterList == null || (_filterList != null && _filterList.Contains(i)))
                {
                    ...
                    // Sinh vị trí mảnh
                    targetPosition.x = puzzleBounds.max.x + decomposeOffset.x ;
                    //targetPosition.x = 8.05f;
                    //targetPosition.y = (puzzleBounds.center.y + decomposeDistance.y / 2 + pieces[i].size.y / 2 - (i));
                    //targetPosition.y = Random.Range(puzzleBounds.center.y + decomposeDistance.y / 2 + pieces[i].size.y / 2, puzzleBounds.center.y - decomposeDistance.y / 2 + pieces[i].size.y / 2) + 10;
                    //Debug.Log("x ====== " + (puzzleBounds.center.y + decomposeDistance.y / 2 + pieces[i].size.y / 2));
                    //Debug.Log("y ====== " + (puzzleBounds.center.y - decomposeDistance.y / 2 + pieces[i].size.y / 2));
    ...
    public void ActivePiece()
    {
       ...
       // }    
        ...
        //    pieces[i].transform.gameObject.SetActive(false);
        //}
        //GameManager.Instance.countPiece += 4;
        //for (int i = pieces.Length; i > GameManager.Instance.countPiece; i --)
        //{
        //    pieces[i].transform.gameObject.SetActive(true);
        //}

        //if ((pieces.Length - GameManager.Instance.countPiece) > 4)
        //{
        //    for (int i = pieces.Length - 1; i >= (pieces.Length - 4); i--)
        //    {
        //        if (pieces[i] != null)
        //        {
        //            pieces[i].transform.gameObject.SetActive(true);
        //        }
        //        //GameManager.Instance.countPiece += 1;
        //    }
        //}
        //else
        //{
        //    for (int j = GameManager.Instance.countPiece; j < pieces.Length; j++)
        //    {
        //        if (pieces[j] != null)
        //        {
        //            pieces[j].transform.gameObject.SetActive(true);
        //        }
        //        //GameManager.Instance.countPiece += 1;
        //    }
        //}
        GameManager.Instance.numberPiece = 5;
        //GameManager.Instance.countPiece += 4;
    }
    //----------------------------------------------------------------------------------------------------
    ...
    public void MovePiece(int _id, Vector3 _targetPosition, bool _useLocalSpace, float _movementTime = -1)
    {
        ...
        pieces[_id].spriteRenderer.drawMode = SpriteDrawMode.Tiled;
        pieces[_id].spriteRenderer.size = new Vector3(1, 1, 1);
        pieces[_id].spriteRenderer.drawMode = SpriteDrawMode.Simple;
        pieces[_id].transform.localScale = GameManager.Instance.getSizePieceFake();

        //pieces[_id].transform.localScale= new Vector3(0.25f, 0.25f, 0.25f);
        if (_movementTime == 0)
        {
            if (_useLocalSpace)
    ...
    public void ReturnPiece(int _id, float _movementTime = -1)
    {
        ...
        pieces[_id].spriteRenderer.drawMode = SpriteDrawMode.Tiled;
        pieces[_id].spriteRenderer.size = pieces[_id].oldSize;
        pieces[_id].spriteRenderer.drawMode = SpriteDrawMode.Simple;

        pieces[_id].transform.localScale = pieces[_id].oldScale;
    }
    ...
    //{
    //    Debug.Log("movedPieces " + movedPieces);
